...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        print("My scope is", self.scope)
        print("url route is", self.scope["url_route"])
        print("kwargs is", self.scope["url_route"]["kwargs"])
        #obtains room_name parameter from the url route in chat/routing.py
        ...
        self.room_group_name = f"chat_{self.room_name}" #construct a Channels group name 
            ...
            # reject them
        #if not user_is_allowed_in_room(user,room)
            # reject them
