...
export default function DemoPaymentPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        console.error(e);
        setError(
          e instanceof Error ? e.message : "Could not load this consultation."
        );
      } finally {
        setLoading(false);
      }
  ...
  async function onPayWithStripe() {
    ...
    try {
      setPaying(true);
      setError(null);
      const { checkout_url } = await createBookingCheckout(booking.id);
  ...
  async function onCompleteDemoPayment() {
    ...
    try {
      setPaying(true);
      setError(null);
      await completeDemoPayment(booking.id);
      navigate("/bookings", {
        replace: true,
        state: {
      ...
      console.error(e);
      setError(
        e instanceof Error ? e.message : "Could not complete demo payment."
      );
    } finally {
      setPaying(false);
    }
  ...
  const stripeEnabled = paymentOptions?.stripe_checkout ?? false;
  const demoEnabled = paymentOptions?.demo_payment ?? false;
  ...
  return (
    <div style={{ maxWidth: 560, margin: "32px auto", padding: "0 16px" }}>
      ...
      {isClient && needsPayment && stripeEnabled && (
        <button
          type="button"
          disabled={paying}
        ...
        >
          {paying ? "Redirecting…" : "Pay consultation fee"}
        </button>
      )}

      {isClient && needsPayment && demoEnabled && (
        <button
          type="button"
          disabled={paying}
          ...
          style={{
            marginTop: 16,
            marginLeft: stripeEnabled ? 8 : 0,
            padding: "10px 14px",
            borderRadius: 8,
            border: "1px solid #d1d5db",
            background: paying ? "#f3f4f6" : "#fff",
            color: "#374151",
            fontWeight: 600,
            cursor: paying ? "not-allowed" : "pointer",
          }}
